...
export default function FloatingLines({
  ...
  // ── Performance: pause flags ──────────────────────────────────
  const visibleRef = useRef(true);   // IntersectionObserver
  const tabActiveRef = useRef(true); // Page Visibility API
  ...
  useEffect(() => {
    ...
    // ── Cap pixel ratio at 1 — background effect doesn't need 2×/3× ──
    ...
    // ── 30fps throttle: render every other frame ──────────────────
    let frameCount = 0;
    ...
    const renderLoop = () => {
      ...
      // Skip rendering if out of view or tab hidden — keeps RAF alive
      // so it can auto-resume without any extra logic
      if (!visibleRef.current || !tabActiveRef.current) return;
